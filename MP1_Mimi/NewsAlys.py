...
#Loopsites -- what's loopsites? did you rename it? -- takes a user-inputted word, creates a gsearch query for each site, and runs gsearch on each query.
...
def newsalys(userin):
     ...
     for site in sites:
          # I went and figured out how to do string interpolation with 2 arguments after our conversation:
          # test = "test string numbers %s %s" % (1,2)
          # The way you're doing it here is completely valid too!
...
##The functions gsearch and artext are here to search for userinput (query?) on the web and process the search results into something that can be analysed.
# gsearch is directed to run a google search of a news site and return the url of the first article on userinput (query?).
def gsearch(query):
     g = Google()
...
def sentmood(article):
     ...
     # -> documentation consistency -- (polarity, subjectivity) is -actually- a tuple, (indicative, imperative...) is just a list of possibilities...but looks like a tuple
...
def positivity(sent0):
     if 0.1 <= sent0 <= 1:
          ...
          return # Great that you're catching the error case!
